package
{
	...
	public class ThemeTest extends Sprite
	{
		...
		private function _handleAddedToStage(ev:Event):void
		{
			...
			_starling.start();
			_themeManager = new CommonThemeWithAssetManager("myTheme","assets/",Objects.assetManager,stage,false);
		...
		private function _handleThemeManagerReady(ev:Event):void
		{
			...
			btn.x = 24;
			btn.y = 96;
			btn.label = "123 ここはFeathersのButtonです";
			addChild(btn);
			
			var label:Label = new Label();
			label.x = 24;
			label.y = 184;
			label.text = "123 ここはFeathersのLabelです";
			addChild(label);
			
			var tf:TextField = new TextField();
			tf.defaultTextFormat = new TextFormat(_themeManager.config.fontName,26,0xff99ff,true);
			tf.text = "123 ここはFlashのTextFieldです";
			tf.selectable = false;
